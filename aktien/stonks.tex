...
\begin{document}
	{
		...
		\setbeamertemplate{background}{\includegraphics[width=\paperwidth,trim=0 0 0 3.4cm]{images/stonks-title-full}}
		\maketitle
		\addtocounter{page}{-1}
	}
...
%				\item[Risiko] Wahrscheinlichkeit des Eintretens eines negativen Ereignisses\pause
		...
		\subsection{Investieren (Geld anlegen)}
			...
			\begin{frame}
				...
				\end{definition}
				\begin{itemize}
					\item Motivation: Glaube an die Qualität\footnote{Wirtschaftliche Qualität, also Rentabilität, Zukunftssicherheit, Markstellung, etc.} des Unternehmens
					\item Horizont: Langfristig (Jahre/Jahrzehnte)
				\end{itemize}\pause\n
				\textbf{Beispiele:}
				\begin{itemize}
					\item Kauf von Aktien zum Vermögensaufbau
					\item Kauf eines Buches → bessere Noten → später mehr Gehalt
%					\item Sport treiben zum Erhalt der körperlichen Gesundheit.
		...
		\subsection{Spekulieren (Trading)}
			...
			\begin{frame}
				\begin{definition}
					Risiko behaftetes Ausnutzen von Preisschwankungen zur Gewinnmitnahme.\citewiki{Spekulation\_(Wirtschaft)}
				\end{definition}
				\begin{itemize}
					\item Motivation: Externe Einflüsse zur Gewinnerzeugung nutzen
					\begin{itemize}
				...
				\textbf{Beispiele:}
				\begin{itemize}
					\item Kauf einer Aktie vor Veröffentlichung eines Quartalsberichts.
					\item Bunkern von Getreide/Rohstoffen in Hoffnung auf Notlagen.
				\end{itemize}
			\end{frame}
...
%			\end{frame}
		...
		\subsection{Fonds}
			...
			\begin{frame}
				...
				Geht diese pleite, verliert ihr kein Geld!\citewiki[Sondervermögen\_(Investmentgesellschaft)]{Sonderverm\%C3\%B6gen\_(Investmentgesellschaft)}
			\end{frame}
			...
			\begin{frame}
				\begin{itemize}
					...
					\begin{itemize}
						...
						\item Hohe TER\footnote{TER = \wikilink{Total\_Expense\_Ratio}{Total Expense Ratio (Gesamtkostenquote)} = jährliche laufende Kosten in \%}: ca. 1,3 - 3,5\%\footnote{z.B. \href{https://www.comdirect.de/inf/fonds/LU0552028770}{Amundi Funds Emerging Markets Equity Focus (ISIN: LU0552028770)}}
					\end{itemize}\pause
					\item Passiv\pause
					\begin{itemize}
			...
			\begin{frame}{Beispiel Workflow}
				...
				\begin{tikzpicture}[->,>=stealth']
					...
					\onslide<3>
					{
						\money{(5.25,1)}
						\money{(5.35,0.9)}
						\draw[thick] (i) to [bend left=10] node[below,align=center,font=\tiny] {z.B. jedes \\ Quartal} (b);
					}
					...
					\onslide<4>
					{
						...
						\node at (8.9,-1.1) {\includegraphics[width=0.75cm]{images/stock}};
						\node[font=\tiny] at (8.9,-1.7) {Aktien};
					}
					...
					\onslide<6>
					{
						...
						\node[font=\tiny] at (1.75,-1.5) {Fondsanteil};
					}
				...
				{
					\textbf{Hinweis:}\\
					Viele andere Variationen/Workflows möglich.
				}
			\end{frame}
			...
			\begin{frame}
				...
				\begin{tikzpicture}[->,>=stealth']
					...
					\onslide<1-8> {
						...
						\node[below right=0.48cm and -2.55cm of b, minimum width=4.5cm, minimum height=0.85cm, draw, black] {};
					}
					\onslide<1-8> {
						\smallmoney{($(i.center)+(-1.25,-0.25)$)}
						\node at ($(i2.center)+(1.25,-0.25)$) {\includegraphics[width=0.5cm]{images/stock}};
					}
...
%						\smallmoney{(4.1, -2.7)}
					...
					\onslide<4-8> {
						...
						\node[below left=0.95cm and 0cm of b, font=\scriptsize, anchor=east, align=right] {Kauforder \\ \tiny 3,45€};
					}
					\onslide<5> {
					...
					\onslide<7-8> {
						\node[below right=0.95cm and 0cm of b, font=\scriptsize, anchor=west, align=left] {Verkaufsorder \\ \tiny 3,42€};
					}
					...
					\onslide<8> {
						\node[below right=0.075cm and -2.675cm of b, font=\tiny] {Orderbuch:};
						\node[below right=0.48cm and -2.55cm of b, minimum width=4.5cm, minimum height=0.85cm, draw, thick, green!75!black] {};
						\draw[<->] (bb)++(-0.5,0) -- node[below] {\checkmark} +(1,0);
						\node[below right=0.75cm and 2cm of b, green!75!black, font=\scriptsize] {Preis passt};
					}
					\onslide<9> {
						\smallmoney{(5, -1.45)}
...
%						\smallmoney{(10, 0.8)}
						\node at ($(i.center)+(-1.25,-0.25)$) {\includegraphics[width=0.5cm]{images/stock}};
						\smallmoney{($(i2.center)+(1.25,-0.25)$)}
					}
				\end{tikzpicture}
			\end{frame}
		...
		\subsection{Informationsquellen}
			...
			\begin{frame}
				...
				\begin{itemize}
					...
					\item \href{https://www.finanztip.de/}{finanztip.de}, \href{https://www.finanzfluss.de/}{finanzfluss.de}, \href{https://www.bafin.de/DE/Verbraucher/verbraucher_node.html}{BaFin}
				\end{itemize}\pause
				YouTube:
				\begin{itemize}
					\item \href{https://www.youtube.com/@finanztip}{Finanztip}, \href{https://www.youtube.com/@Finanzfluss}{Finanzfluss}, \href{https://www.youtube.com/@MadameMoneypenny}{Madame Moneypenny}
				\end{itemize}\pause
				Bücher:
				\begin{itemize}
